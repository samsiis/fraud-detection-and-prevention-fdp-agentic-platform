...
class Variables:
    ...
    def get_fdp_env(self, value) -> str:
        if os.getenv(value):
    ...
    def get_fdp_secret(self, secret, value) -> str:
        response = None
        if os.getenv(secret):
            response = self._retrieve_from_secretsmanager(os.getenv(secret))
            if response and value in response:
                return response[value]
        return self.get_fdp_env(value)

    def get_fdp_logging(self) -> str:
        result = self.get_fdp_env('FDP_LOGGING')
        if result == 'DEBUG':
